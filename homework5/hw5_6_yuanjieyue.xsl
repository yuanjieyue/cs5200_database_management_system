...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/ReviewApplication">
        ...
        <h1>user5 Recommendations</h1>
        <!-- Determine the count of Recommendations made by “username5”. -->
        <table border="1">
            <tr>
                <th>UserName</th>
            ...
            <xsl:for-each select="Recommendations/Recommendation">
                <xsl:variable name="username" value="user5"></xsl:variable>
                <xsl:variable name="recommendtions" 
                    select="count(/ReviewApplication/Recommendations/Recommendation/Users/User[UserName/text()=$username])"></xsl:variable>
                <tr>
                    <td>
                        <xsl:value-of select="$username/text()"></xsl:value-of>
                    </td>
                    <td>
                        <xsl:value-of select="$recommendtions/text()"></xsl:value-of>
                    </td>
                </tr>
            </xsl:for-each>
        </table>
