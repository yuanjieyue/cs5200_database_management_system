...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/ReviewApplication">
        ...
        <!-- Now “unroll” the array of Restaurant Names so each row contains exactly one CompanyName and Restaurant Name. -->        
        <table border="1">
            <tr>
                <th>CompanyName</th>
                <th>Restaurants</th>
            ...
            <xsl:for-each select="Companies/Company">
                ...
                <xsl:for-each select="Restaurants/Restaurant">
                    <xsl:variable name="restaurant" select="/ReviewApplication/Restaurants/Restaurant[CompanyName/text() = $companyname]/Name"></xsl:variable>
                    <tr>
                        <td>
