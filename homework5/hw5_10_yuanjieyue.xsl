...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/ReviewApplication">
        ...
        <!-- For each Review, display the Restaurant Name, User’s FirstName, and Review Rating. -->
        <table border="1">
            <tr>
            ...
            <xsl:for-each select="Reviews/Review">
                <xsl:variable name="restaurantname" select="RestaurantId"></xsl:variable>
                <xsl:variable name="username" select="UserName"></xsl:variable>
                <xsl:variable name="firstname" 
                select="/ReviewApplication/Users/User[FirstName/text() = $username]/FirstName"></xsl:variable>
                ...
                <tr>
                    ...
                    <td>
                        <xsl:value-of select="$firstrname/text()"></xsl:value-of>
                    </td>
                    <td>
