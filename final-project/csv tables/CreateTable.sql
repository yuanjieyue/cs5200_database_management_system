...
CREATE TABLE BoardGames(
  ...
  AverageTime INT,
  YearReleased INT,
  RatingScore Decimal(10,2),
  Mechanics VARCHAR(1024),
  Owned VARCHAR(255),
  Theme VARCHAR(255),
...
CREATE TABLE Orders(
  ...
  UserId INT,
  GameId INT,
  PlaceOrderDate TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  ReceiverName VARCHAR(255),
  ShipAddress VARCHAR(255),
  ShippedDate TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  PaymentMethod VARCHAR(255),
  OrderStatus ENUM ('new', 'hold', 'shipped', 'delievered', 'cancelled'),
  ShippingOptions enum('ground', 'express', 'twodays', 'oneday'),
